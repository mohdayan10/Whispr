 ...
 * WhisprProSTT
 *
 * Connects to the Whispr API WebSocket transcription endpoint.
 ...
 */
export class WhisprProSTT extends EventEmitter {
    ...
    private readonly BACKEND_URL = 'wss://api.invalid/v1/transcribe';
    ...
    public setSampleRate(rate: number): void {
        ...
        console.log(`[WhisprProSTT:${this.channel}] Sample rate configured to ${rate}Hz`);
    }
    ...
    public setRecognitionLanguage(key: string): void {
        ...
        if (key === 'auto') {
            ...
            console.log('[WhisprProSTT] Language set to auto-detect mode');
        } else {
            const config = RECOGNITION_LANGUAGES[key];
            if (!config) {
                console.warn(`[WhisprProSTT] Unknown language key: ${key}`);
                return;
            }
            ...
            this.languageAlternates = 'alternates' in config
                ...
                : [];
            console.log(`[WhisprProSTT] Language set: ${key} → ${this.languageBcp47}`,
        ...
        if (this.isActive && this.ws) {
            console.log('[WhisprProSTT] Language changed while active — reconnecting');
    ...
    /** No-op — Whispr API server handles VAD internally */
    ...
    public write(chunk: Buffer): void {
        ...
        if (!this.isConnected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
            ...
            if (this.buffer.length <= 3 || this.buffer.length % 100 === 0) {
                ...
                console.log(`[WhisprProSTT:${this.channel}] Buffering chunk (buffer=${this.buffer.length}, isConnected=${this.isConnected}, ws=${wsState})`);
            }
            return;
        }
        ...
        if (this._chunksSent <= 5 || this._chunksSent % 200 === 0) {
            console.log(`[WhisprProSTT:${this.channel}] Sent chunk #${this._chunksSent} (${chunk.length}B) to server`);
        }
        this.ws.send(chunk);
    }
    ...
    private connect(): void {
        ...
        console.log(`[WhisprProSTT] Connecting (attempt ${this.reconnectAttempts + 1})...`);
        ...
        this.ws.on('message', (data: WebSocket.Data) => {
            try {
                ...
                if (!msg.text || msg.is_final) {
                    console.log(`[WhisprProSTT:${this.channel}] Server msg:`, JSON.stringify(msg).slice(0, 120));
                }

                if (msg.error) {
                    console.error('[WhisprProSTT] Server error:', msg.error, msg.message || '');
                ...
                if (msg.status === 'connected') {
                    ...
                    this.reconnectAttempts = 0;
                    console.log(`[WhisprProSTT] Connected via ${msg.provider}`);
                    this.flushBuffer();
                    return;
                }
                ...
                if (msg.language_detected) {
                    ...
                    console.log(`[WhisprProSTT] Auto-detected language: ${detected}`);
                ...
                console.error('[WhisprProSTT] Parse error:', err);
            }
        });

        this.ws.on('error', (err: Error) => {
            console.error('[WhisprProSTT] WebSocket error:', err.message);
            this.isConnecting = false;
            this.isConnected  = false;
            this.emit('error', err);
        });
        ...
        this.ws.on('close', (code: number) => {
            this.isConnecting = false;
            this.isConnected  = false;
            console.log(`[WhisprProSTT] Connection closed (code ${code})`);
    ...
    private scheduleReconnect(): void {
        ...
        if (this.reconnectAttempts >= this.MAX_RECONNECT) {
            console.error('[WhisprProSTT] Max reconnect attempts reached — giving up');
            this.emit('error', new Error('WhisprProSTT: max reconnect attempts exceeded'));
            return;
        }

        const delay = this.RECONNECT_BASE_MS * Math.pow(2, this.reconnectAttempts);
        this.reconnectAttempts++;
        console.log(`[WhisprProSTT] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.MAX_RECONNECT})...`);
