...
export interface StoredCredentials {
    ...
    sttProvider?: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'whispr';
    ...
    // Free trial state
    trialToken?:     string;   // server-issued signed token (whispr_trial_…)
...
export class CredentialsManager {
    ...
    public getSttProvider(): 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'whispr' {
    ...
    public getWhisprApiKey(): string | undefined {
        return this.credentials.whisprApiKey;
    }
    ...
    public setSttProvider(provider: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'whispr'): void {
    ...
    public setWhisprApiKey(key: string): void {
        const trimmed = key.trim();
        this.credentials.whisprApiKey = trimmed || undefined;

        if (trimmed) {
            // Auto-promote whispr to default model unless user already chose a non-Gemini/Groq model
            ...
            const isAutoDefault = !current
                ...
                || current === 'llama';
            if (isAutoDefault) {
                this.credentials.defaultModel = 'whispr';
                console.log('[CredentialsManager] Auto-set default model to whispr');
            }

            // Auto-promote whispr STT if still on 'none' or the default Google STT
            if (!this.credentials.sttProvider || this.credentials.sttProvider === 'none' || this.credentials.sttProvider === 'google') {
                this.credentials.sttProvider = 'whispr';
                console.log('[CredentialsManager] Auto-set STT provider to whispr');
            }
        } else {
            // Key cleared — revert whispr-auto-set defaults back to safe fallbacks
            if (this.credentials.defaultModel === 'whispr') {
                this.credentials.defaultModel = 'gemini-3.1-flash-lite-preview';
                console.log('[CredentialsManager] Whispr key cleared — reset default model to Gemini Flash');
            }
            if (this.credentials.sttProvider === 'whispr') {
                this.credentials.sttProvider = 'none';
                console.log('[CredentialsManager] Whispr key cleared — reset STT provider to none');
            }
        }

        this.saveCredentials();
        console.log('[CredentialsManager] Whispr API Key updated');
    }
