...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  const isProOrTrialActive = (): boolean => {
    // 1. Full premium license (Dodo / Gumroad / Whispr API subscription)
    try {
  ...
  safeHandle(
    ...
    async (event, { width, height }: { width: number; height: number }) => {
      ...
      if (settingsWin && !settingsWin.isDestroyed() && settingsWin.webContents.id === senderWebContents.id) {
        ...
        // WhisprInterface logic - Resize ONLY the overlay window using dedicated method
  ...
  // Generate suggestion from transcript - Whispr-style text-only reasoning
  ...
  safeHandle("get-log-file-path", async () => {
    try {
      return path.join(app.getPath('documents'), 'whispr_debug.log');
    } catch {
      return null;
    }
  });
  ...
  safeHandle("open-log-file", async () => {
    try {
      const logPath = path.join(app.getPath('documents'), 'whispr_debug.log');
  ...
  safeHandle("set-whispr-api-key", async (_, apiKey: string) => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
      const cm = CredentialsManager.getInstance();
      const prevSttProvider = cm.getSttProvider();
      cm.setWhisprApiKey(apiKey);

      // Update LLMHelper immediately (same pattern as other provider keys)
      const llmHelper = appState.processingHelper.getLLMHelper();
      llmHelper.setWhisprKey(apiKey || null);
      ...
      // If setWhisprApiKey auto-promoted the STT provider to 'whispr', reconfigure
      ...
      if (newSttProvider !== prevSttProvider) {
        console.log(`[IPC] set-whispr-api-key: STT provider changed ${prevSttProvider} → ${newSttProvider}, reconfiguring pipeline`);
        await appState.reconfigureSttProvider();
      }

      // Auto-activate Whispr Pro for pro/max/ultra API plans.
      ...
      if (apiKey) {
        try {
          ...
          if (result.success) {
            console.log('[IPC] set-whispr-api-key: Pro auto-activated via API plan.');
            ...
            console.log('[IPC] set-whispr-api-key: existing Gumroad/Dodo license preserved — Pro not overwritten.');
          } else {
            console.log('[IPC] set-whispr-api-key: Pro not activated —', result.error);
          }
        } catch (e: any) {
          // LicenseManager not available in this build — non-fatal
          console.warn('[IPC] set-whispr-api-key: LicenseManager unavailable for Pro auto-activation:', e?.message);
        }
      } else {
        // API key was cleared — deactivate any whispr_api Pro license so premium is revoked.
        try {
          const { LicenseManager } = require('../premium/electron/services/LicenseManager');
          const lm = LicenseManager.getInstance();
          // Only deactivate if the stored license is from a whispr_api subscription.
          // Never touch Gumroad/Dodo lifetime licenses here.
          const details = lm.getLicenseDetails();
          if (details.isPremium && details.provider === 'whispr_api') {
            await lm.deactivate();
            console.log('[IPC] set-whispr-api-key: key cleared — whispr_api Pro license deactivated.');
            BrowserWindow.getAllWindows().forEach(win => {
              if (!win.isDestroyed()) win.webContents.send('license-status-changed', { isPremium: false });
            });
          }
        } catch (e: any) {
          console.warn('[IPC] set-whispr-api-key: LicenseManager unavailable for Pro deactivation on key clear:', e?.message);
        }
      }

      return { success: true };
    } catch (error: any) {
      console.error("Error saving Whispr API key:", error);
      return { success: false, error: error.message };
    } finally {
  ...
  safeHandle("get-whispr-usage", async () => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
      const key = CredentialsManager.getInstance().getWhisprApiKey();
      ...
      const res = await fetch('https://api.invalid/v1/usage', {
        headers: { 'x-whispr-key': key },
  ...
  safeHandle("invalidate-whispr-usage-cache", () => {
    _usageCache.clear();
    return { ok: true };
  });
  ...
  safeHandle("trial:start", async () => {
    try {
      ...
      const res = await fetch('https://api.invalid/v1/trial/start', {
        method:  'POST',
      ...
      if (data.ok && data.trial_token && !data.expired) {
        ...
        // Auto-configure whispr as the model + STT provider during trial
        const prevSttProvider = cm.getSttProvider();
        cm.setWhisprApiKey('__trial__');   // sentinel — activates whispr model routing
        const newSttProvider = cm.getSttProvider();
        if (newSttProvider !== prevSttProvider) {
          await appState.reconfigureSttProvider();
        }
        const llmHelper = appState.processingHelper?.getLLMHelper?.();
        if (llmHelper) llmHelper.setWhisprKey('__trial__');
      }
  ...
  safeHandle("trial:status", async () => {
    try {
      ...
      const res = await fetch('https://api.invalid/v1/trial/status', {
  ...
  safeHandle("trial:convert", async (_, choice: string) => {
    try {
      ...
      await fetch('https://api.invalid/v1/trial/convert', {
        method:  'POST',
  ...
  // End trial via BYOK path: wipe Pro-ingested data, clear trial token + whispr key.
  safeHandle("trial:end-byok", async () => {
    try {
      ...
      const token = cm.getTrialToken();
      if (token) {
        fetch('https://api.invalid/v1/trial/convert', {
          method:  'POST',
          headers: { 'Content-Type': 'application/json', 'x-trial-token': token },
      ...
      cm.setWhisprApiKey('');
      const llmHelper = appState.processingHelper?.getLLMHelper?.();
      if (llmHelper) llmHelper.setWhisprKey(null);
      await appState.reconfigureSttProvider();
  ...
  // trial token or whispr key. Called automatically when trial expires so that
  ...
  safeHandle("get-stored-credentials", async () => {
    try {
      ...
      return {
        ...
        hasWhisprKey: hasKey(creds.whisprApiKey),
  ...
  safeHandle("set-stt-provider", async (_, provider: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'whispr') => {
    try {
      const { CredentialsManager } = require('./services/CredentialsManager');
  ...
  safeHandle("set-model", async (_, modelId: string) => {
    try {
      ...
      // Broadcast to all windows so WhisprInterface can update its selector (session-only update)
      BrowserWindow.getAllWindows().forEach(win => {
        if (!win.isDestroyed()) {
  ...
  safeHandle("set-default-model", async (_, modelId: string) => {
    try {
      ...
      // Broadcast to all windows so WhisprInterface can update its selector
      BrowserWindow.getAllWindows().forEach(win => {
        if (!win.isDestroyed()) {
          win.webContents.send('model-changed', modelId);
        }
  ...
  safeHandle("profile:research-company", async (_, companyName: string) => {
    try {
      ...
      // Wire search provider: Tavily (user key) → Whispr API (fallback) → none (LLM-only)
      const { CredentialsManager } = require('./services/CredentialsManager');
      const cm = CredentialsManager.getInstance();
      ...
      if (tavilyApiKey) {
        ...
        const whisprKey = cm.getWhisprApiKey();
        if (whisprKey) {
          const { WhisprSearchProvider } = require('../premium/electron/knowledge/WhisprSearchProvider');
          ...
          const trialToken = whisprKey === '__trial__' ? cm.getTrialToken() : undefined;
          engine.setSearchProvider(new WhisprSearchProvider(whisprKey, trialToken ?? undefined));
          console.log('[IPC] Company research: using Whispr API search (no Tavily key configured)');
        }
      }
