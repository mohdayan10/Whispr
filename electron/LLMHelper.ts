...
export class LLMHelper {
  ...
  private whisprKey: string | null = null;
  ...
  public setWhisprKey(key: string | null): void {
    this.whisprKey = key || null;
    console.log(`[LLMHelper] Whispr key ${key ? 'set' : 'cleared'}`);
  }

  private hasWhispr(): boolean {
    return !!this.whisprKey;
  }
...
- Do NOT wrap the JSON in markdown fences.`;
  ...
  /**
   * Generate a suggestion based on conversation transcript - Whispr-style
...
[END LANGUAGE OVERRIDE]\n\n`;
  ...
  public async chatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, alternateGroqMessage?: string): Promise<string> {
    try {
      ...
      if (this.currentModelId === 'whispr') {
        const { CredentialsManager } = require('./services/CredentialsManager');
        const whisprKey = CredentialsManager.getInstance().getWhisprApiKey();
        if (whisprKey) {
          try {
            return await this.generateWithWhispr(userContent, openaiSystemPrompt, imagePaths);
          } catch (err: any) {
            console.warn('[LLMHelper] Whispr API failed in chatWithGemini, falling back to Gemini:', err.message);
      ...
      if (isMultimodal) {
        // MULTIMODAL PROVIDER ORDER: [Whispr] -> OpenAI -> Gemini Flash -> Claude -> Gemini Pro -> Groq -> Custom/Ollama
        if (this.hasWhispr()) {
          providers.push({ name: 'Whispr API', execute: () => this.generateWithWhispr(userContent, openaiSystemPrompt, imagePaths) });
        }
        ...
        // TEXT-ONLY: [Whispr] -> Groq -> Gemini Flash -> Gemini Pro -> OpenAI -> Claude
        if (this.hasWhispr()) {
          providers.push({ name: 'Whispr API', execute: () => this.generateWithWhispr(userContent, openaiSystemPrompt) });
        }
        if (this.groqClient) {
  ...
  public async generateContentStructured(message: string): Promise<string> {
    ...
    // Priority 7: Whispr API — used when no other provider is available, or as final fallback
    const whisprKeyForStructured = this.whisprKey || (() => {
      try { return require('./services/CredentialsManager').CredentialsManager.getInstance().getWhisprApiKey() || null; } catch { return null; }
    })();
    if (whisprKeyForStructured) {
      providers.push({
        name: 'Whispr API',
        execute: () => this.generateWithWhispr(message)
      });
    }

    if (providers.length === 0) {
      throw new Error('No reasoning model available. Please configure an OpenAI, Claude, Gemini, Groq, or Whispr API key.');
    }
   ...
   */
  /**
   * Routes AI generation through the Whispr API backend (Gemini-powered).
   */
  private async generateWithWhispr(userMessage: string, systemPrompt?: string, imagePaths?: string[]): Promise<string> {
    // Prefer the in-memory field; fall back to CredentialsManager for the direct-routing path
    // where currentModelId === 'whispr' but setWhisprKey() wasn't called yet.
    let whisprKey = this.whisprKey;
    if (!whisprKey) {
      const { CredentialsManager } = require('./services/CredentialsManager');
      whisprKey = CredentialsManager.getInstance().getWhisprApiKey() || null;
    }
    if (!whisprKey) throw new Error('Whispr API key not set');

    const endpointUrl = 'https://api.invalid/v1/chat';
    ...
    if (whisprKey === '__trial__') {
      ...
      headers['x-whispr-key'] = whisprKey;
    }
    ...
    //
    // Compress before sending: retina screenshots are 2-5 MB PNG; the Whispr API body limit
    ...
    if (!response.ok) {
      ...
      throw new Error(`Whispr API error ${response.status}: ${errData.error || 'unknown'}`);
    }
  ...
  public async * streamChatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false): AsyncGenerator<string, void, unknown> {
    ...
    if (isMultimodal) {
      // MULTIMODAL PROVIDER ORDER: [Whispr] -> OpenAI -> Gemini Flash -> Claude -> Gemini Pro -> Groq Scout 4
      if (this.hasWhispr()) {
        providers.push({ name: 'Whispr API', execute: () => this.streamWithWhispr(userContent, openaiSystemPrompt, imagePaths) });
      }
      if (this.openaiClient) {
      ...
      // TEXT-ONLY PROVIDER ORDER: [Whispr] → Groq → OpenAI → Claude → Gemini Flash → Gemini Pro
      if (this.hasWhispr()) {
        providers.push({ name: 'Whispr API', execute: () => this.streamWithWhispr(userContent, openaiSystemPrompt) });
      }
      if (this.groqClient) {
    ...
    // ============================================================
    const currentFamilyLabel = this.currentModelId === 'whispr' ? 'Whispr'
    ...
    // Whispr is always first when configured, regardless of which model is selected.
    // The sort above may have displaced it — restore it to position 0.
    if (this.hasWhispr() && providers[0]?.name !== 'Whispr API') {
      const idx = providers.findIndex(p => p.name === 'Whispr API');
      if (idx > 0) {
  ...
  public async * streamChat(
    ...
    // GROQ FAST TEXT OVERRIDE (Text-Only)
    // Two paths: local Groq key → call Groq directly; Whispr API only → send fast_mode:true
    ...
    if (this.groqFastTextMode && !isMultimodal) {
      if (this.groqClient) {
        ...
        // Local Groq failed — fall through to Whispr if available
      }
      if (this.hasWhispr()) {
        // streamWithWhispr → generateWithWhispr → sends fast_mode:true → server Groq pool
        console.log(`[LLMHelper] ⚡️ Groq Fast Text Mode Active (Streaming). Routing to Whispr server Groq pool...`);
        try {
          yield* this.streamWithWhispr(userContent, finalSystemPrompt);
          return;
        } catch (e: any) {
          console.warn("[LLMHelper] Whispr fast-mode failed, falling back:", e.message);
        }
      }
    }
    ...
    if (this.currentModelId === 'whispr') {
      ...
      if (whisprKey) {
        try {
          const response = await this.generateWithWhispr(userContent, finalSystemPrompt, imagePaths);
          yield response;
          return;
        } catch (err: any) {
          console.warn('[LLMHelper] Whispr API failed in streamChat, trying Groq fallback:', err.message);
    ...
    // 5. Last-resort: Whispr API (if user has a key but no cloud provider configured)
    if (this.hasWhispr()) {
      try {
        yield* this.streamWithWhispr(userContent, finalSystemPrompt, imagePaths);
        return;
      } catch (e: any) {
        console.warn('[LLMHelper] Whispr last-resort fallback failed:', e.message);
      }
    }
  ...
  /**
   * Fake-stream for Whispr API (non-streaming endpoint).
   ...
   */
  private async * streamWithWhispr(userContent: string, systemPrompt?: string, imagePaths?: string[]): AsyncGenerator<string, void, unknown> {
    // ── REAL SSE STREAM (replaces the fake word-by-word simulation) ──────────
    // Previous implementation called generateWithWhispr() (blocking, waited for
    ...
    if (whisprKey === '__trial__') {
      ...
      streamHeaders['x-whispr-key'] = whisprKey;
    }
    ...
    const response = await fetch('https://api.invalid/v1/chat', {
      method:  'POST',
    ...
    if (!response.ok) {
      ...
      throw new Error(`Whispr API ${response.status}: ${(errData as any).error || 'unknown'}`);
    }
  ...
  public async generateMeetingSummary(systemPrompt: string, context: string, groqSystemPrompt?: string): Promise<string> {
    ...
    // ATTEMPT 1: Whispr API (if configured — first in chain)
    if (this.hasWhispr()) {
      try {
        console.log(`[LLMHelper] Attempting Whispr API for summary...`);
        const text = await this.withTimeout(
          this.generateWithWhispr(`Context:\n${context}`, systemPrompt),
          60000,
          'Whispr Summary'
        );
        if (text.trim().length > 0) {
          console.log(`[LLMHelper] ✅ Whispr API summary generated successfully.`);
          return this.processResponse(text);
        }
      } catch (e: any) {
        console.warn(`[LLMHelper] ⚠️ Whispr API summary failed: ${e.message}. Falling back...`);
      }
    }
