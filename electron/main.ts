...
const getLogFile = (): string | null => {
  ...
  try {
    _logFile = path.join(app.getPath('documents'), 'whispr_debug.log');
    return _logFile;
  } catch {
...
import { WhisprProSTT } from "./audio/WhisprProSTT"
...
type STTProvider = (GoogleSTT | RestSTT | DeepgramStreamingSTT | SonioxStreamingSTT | ElevenLabsStreamingSTT | OpenAIStreamingSTT | WhisprProSTT) & {
  finalize?: () => void;
...
export class AppState {
  ...
  public async quitAndInstallUpdate(): Promise<void> {
    ...
    if (process.platform === 'darwin') {
      try {
        // Get the downloaded update file path (e.g., .../Whispr-1.0.9-mac.zip)
  ...
  private createSTTProvider(speaker: 'interviewer' | 'user'): STTProvider | null {
    ...
    if (sttProvider === 'whispr') {
      const whisprKey = CredentialsManager.getInstance().getWhisprApiKey();
      if (!whisprKey) {
        // Whispr is Coming Soon — no key means degrade gracefully like every other provider
        console.warn(`[Main] No Whispr API Key configured for ${speaker}, falling back to GoogleSTT`);
        ...
        stt = new WhisprProSTT(whisprKey, speaker === 'interviewer' ? 'system' : 'mic');
      }
    ...
    // Auto language detection: WhisprProSTT emits 'languageDetected' when the
    ...
    if (stt instanceof WhisprProSTT) {
  ...
  public async startMeeting(metadata?: any): Promise<void> {
    ...
    if (process.platform === 'darwin') {
      ...
      if (screenStatus === 'denied') {
        ...
        const message = 'Screen Recording permission denied. System audio will not be captured. To fix: System Settings → Privacy & Security → Screen Recording → enable Whispr.';
  ...
  public setRecognitionLanguage(key: string): void {
    ...
    // 'auto' is only meaningful for WhisprProSTT — other providers fall back to en-US.
    const sttProvider = CredentialsManager.getInstance().getSttProvider();
    const effectiveKey = (key === 'auto' && sttProvider !== 'whispr') ? 'english-us' : key;
  ...
  public showTray(): void {
    ...
    this.tray.setToolTip('Whispr') // This tooltip might also need update if we change global shortcut, but global shortcut is removed.
    this.updateTrayMenu();
  ...
  public setUndetectable(state: boolean): void {
    ...
    if (process.platform === 'darwin') {
      ...
      this._dockDebounceTimer = setTimeout(() => {
        ...
        if (settled) {
          // Capture whether Whispr is currently the frontmost app BEFORE
          ...
          // If Whispr was the focused window when the user toggled stealth,
          ...
          if (whisprWasFocused && targetFocusWindow && !targetFocusWindow.isDestroyed()) {
  ...
  private _applyDisguise(mode: 'terminal' | 'settings' | 'activity' | 'none'): void {
    ...
    switch (mode) {
      ...
      case 'none':
        ...
        if (isMac) {
          iconPath = app.isPackaged
            ? path.join(process.resourcesPath, "whispr.icns")
            : path.join(app.getAppPath(), "assets/whispr.icns");
        } else if (isWin) {
          iconPath = app.isPackaged
    ...
    if (isWin) {
      ...
      app.setAppUserModelId(`com.whispr.assistant.${mode}`);
    }
...
async function initializeApp() {
  ...
  //   1. The Whispr launcher window is visible and focused when the TCC dialog
          ...
          BrowserWindow.getAllWindows().forEach((win: Electron.BrowserWindow) => {
            if (!win.isDestroyed()) {
              win.webContents.send(
                ...
                'Screen Recording is disabled. System audio capture will not work. Click "Open Settings" to enable it, then restart Whispr.'
              );
            }
          });
        } else {
