...
import WhisprInterface from "./components/WhisprInterface"
...
import { WhisprQuotaBanner } from "./components/WhisprQuotaBanner"
...
const App: React.FC = () => {
  ...
  // API check
  const [hasWhisprApi, setHasWhisprApi] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // Also check for Whispr API key
    window.electronAPI?.getStoredCredentials?.()
      .then((creds) => setHasWhisprApi(!!creds?.hasWhisprKey))
      .catch(() => {});
    ...
    // ── Onboarding toasters ──────────────────────────────────
    if (isLauncherWindow || isDefault) {
      const permsShown = localStorage.getItem('whispr_perms_shown_v1');
      if (!permsShown) {
  ...
  const handleStartMeeting = async () => {
    try {
      localStorage.setItem('whispr_last_meeting_start', Date.now().toString());
  ...
  const handleEndMeeting = async () => {
    ...
    try {
      ...
      const startStr = localStorage.getItem('whispr_last_meeting_start');
      if (startStr) {
        const duration = Date.now() - parseInt(startStr, 10);
        const threshold = import.meta.env.DEV ? 10000 : 180000;
        if (duration >= threshold) {
          localStorage.setItem('whispr_show_profile_toaster', 'true');
        }
        localStorage.removeItem('whispr_last_meeting_start');
      }
  ...
  return (
    ...
    <div className="h-full min-h-0 w-full relative bg-[#000000]">
      ...
      <PermissionsToaster
        ...
        onDismiss={() => {
          localStorage.setItem('whispr_perms_shown_v1', '1');
      ...
      {(isLauncherMainView || !!activeAd) && !isSettingsOpen && (
        <WhisprApiPromoToaster
          isOpen={activeAd === 'whispr_api'}
          onDismiss={() => dismissAd('whispr_api')}
