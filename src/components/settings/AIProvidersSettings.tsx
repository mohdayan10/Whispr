...
export const AIProvidersSettings: React.FC = () => {
    ...
    // Fast mode is available with a local Groq key OR via the Whispr API (server-side Groq pool)
    const canUseFastMode = !!(hasStoredKey.groq || hasStoredKey.whispr);
            ...
            try {
                ...
                if (creds) {
                    setHasStoredKey({
                        ...
                        whispr: creds.hasWhisprKey || false
                    });
    ...
    // Effect to enforce fast mode disabled if neither Groq key nor Whispr API is configured.
    ...
    useEffect(() => {
        ...
        if (!canUseFastMode && fastResponseMode) {
            ...
            localStorage.setItem('whispr_groq_fast_text', 'false');
            // @ts-ignore
        ...
        <div className="space-y-5 animated fadeIn pb-10">
            ...
            <div className="space-y-5">
                ...
                <div className="bg-bg-item-surface rounded-xl p-5 border border-border-subtle flex items-center justify-between">
                    ...
                    <ModelSelect
                        ...
                        options={(() => {
                            ...
                            if (hasStoredKey.whispr) {
                                opts.push({ id: 'whispr', name: 'Whispr API' });
                            }
                ...
                <div
                    ...
                    title={!canUseFastMode ? "Requires a Groq API Key or Whispr API to be configured" : ""}
                >
                    <div>
                        ...
                        {!canUseFastMode && (
                            <p className="text-[10px] text-orange-500 mt-0.5 font-medium">Requires a Groq API Key or Whispr API to be configured.</p>
                        )}
                    </div>
                    <div
                        onClick={async () => {
                            if (!canUseFastMode) {
                                alert("Please configure a Groq API Key or Whispr API first to enable Fast Response Mode.");
                                return;
                            }
                            const newState = !fastResponseMode;
                            setFastResponseMode(newState);
                            localStorage.setItem('whispr_groq_fast_text', String(newState));
                            // @ts-ignore
