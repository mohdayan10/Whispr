...
import { WhisprLogoMark } from '../WhisprLogoMark';
...
// ─── Component ───────────────────────────────────────────────
export const WhisprApiSettings: React.FC = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (creds.hasWhisprKey) { setApiKey('•'.repeat(24)); setIsSaved(true); }
            } catch (e) { console.error('[WhisprApi]', e); }
    ...
    const fetchUsage = useCallback(async () => {
        ...
        setUsageError(null);
        try {
            const r = await window.electronAPI.getWhisprUsage();
            if (r.ok && r.quota) {
    ...
    const handleSave = async () => {
        ...
        try {
            const r = await window.electronAPI.setWhisprApiKey(apiKey.trim());
            if (r.success) {
                setApiKey('•'.repeat(24)); setIsSaved(true); setJustSaved(true);
                setTimeout(() => setJustSaved(false), 2500);
                // @ts-ignore
                window.electronAPI?.setDefaultModel?.('whispr').catch(console.error);
                // @ts-ignore
                window.electronAPI?.setSttProvider?.('whispr').catch(console.error);
    ...
    const handleClear = () => {
        ...
        window.electronAPI.setWhisprApiKey('').catch(() => {});
    };
            ...
            <div className="px-5 pt-5 pb-2">
                <div className="flex flex-col gap-2.5 mb-4">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-text-tertiary">Pro, Max &amp; Ultra include Whispr Pro app</span>
                    </div>
    ...
    return (
        <div className="space-y-4 animated fadeIn">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-[15px] font-semibold text-text-primary tracking-[-0.01em]">Whispr API</h3>
                ...
                return (
                    <Card className="shadow-sm border-violet-500/25">
                        <div className="px-5 pt-5 pb-5 space-y-4">
                            {/* Header — same layout as "Try Whispr API free" start card */}
                            <div className="flex items-start gap-3.5">
                                <div className="w-10 h-10 rounded-[11px] bg-violet-500/10 border border-violet-500/20 flex items-center justify-center shrink-0">
                                    <WhisprLogoMark size={18} className="text-violet-400" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between">
                    ...
                    <Card className="shadow-sm">
                        <div className="px-5 pt-5 pb-4 flex flex-col items-center justify-center text-center">
                            ...
                            <div className="w-[42px] h-[42px] mb-3 rounded-[12px] bg-bg-input border border-border-subtle shadow-[inset_0_1px_rgba(255,255,255,0.06),0_2px_8px_rgba(0,0,0,0.04)] flex items-center justify-center relative overflow-hidden">
                                <WhisprLogoMark size={20} className={isClaimed ? "text-text-tertiary" : "text-text-primary drop-shadow-sm"} />
                            </div>
                            
                            <h3 className="text-[14.5px] font-bold text-text-primary tracking-tight mb-1">Whispr API. Try it free.</h3>
            ...
            <Card>
                ...
                <div className="flex items-center gap-3 px-5 pt-5 pb-4">
                    ...
                    <div className="w-9 h-9 rounded-xl bg-blue-500/15 border border-blue-500/20 flex items-center justify-center shrink-0">
                        <WhisprLogoMark size={18} className="text-blue-400" />
                    </div>
                    <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-text-tertiary leading-snug mt-0.5">
                            Your Whispr API key from your subscription email
                        </p>
                    </div>
                </div>
            ...
            <Card>
                <div className="px-5 py-4">
                    <div className="flex items-center justify-between mb-3.5">
                        ...
                        <button
                            onClick={() => openExternal('about:blank')}
                    ...
                    <div className="space-y-3">
                        {[
                            ...
                            { step: '3', text: 'Paste it here — Whispr handles the rest automatically.' },
                        ].map(({ step, text }) => (
