...
interface WhisprInterfaceProps {
    onEndMeeting?: () => void;
    overlayOpacity?: number;
}

const WhisprInterface: React.FC<WhisprInterfaceProps> = ({ onEndMeeting, overlayOpacity = OVERLAY_OPACITY_DEFAULT }) => {
    ...
    const [hideChatHidesWidget, setHideChatHidesWidget] = useState(() => {
        const stored = localStorage.getItem('whispr_hideChatHidesWidget');
    ...
    // Persist Settings
    useEffect(() => {
        localStorage.setItem('whispr_undetectable', String(isUndetectable));
        localStorage.setItem('whispr_hideChatHidesWidget', String(hideChatHidesWidget));
    ...
    const handleAnswerNow = async () => {
        if (isManualRecording) {
            ...
            window.electronAPI.finalizeMicSTT().catch(err => console.error('[WhisprInterface] Failed to send finalizeMicSTT:', err));
...
export default WhisprInterface;
