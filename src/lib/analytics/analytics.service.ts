...
class AnalyticsService {
    ...
    public initAnalytics(): void {
        ...
        if (!GA4_MEASUREMENT_ID) {
            // No measurement ID configured — analytics disabled.
    ...
    public trackAppOpen(): void {
        ...
        const hasLaunched = localStorage.getItem('whispr_has_launched');
        if (!hasLaunched) {
            this.trackEvent('first_launch');
            localStorage.setItem('whispr_has_launched', 'true');
        }
    }
